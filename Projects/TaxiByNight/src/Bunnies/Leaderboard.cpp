...
void Leaderboard::SendPlayerPoints(int id, const std::string& playerName, float reward, int courses)
{
	...
	char request[2048];
	sprintf(request, AddUserAddress.c_str(), id, StringUtils::ToNarrow(base64String.GetPointer()).c_str(), StringUtils::ToBase64(StringUtils::ToString(reward)).c_str(), courses);
