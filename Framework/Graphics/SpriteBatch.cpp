...
float SpriteBatch::VertexBuffer[16];
		  ...
		  const unsigned char *colorMask)
{
	...
	memcpy((void*)VertexBuffer, verts, size);
	memcpy((char*)VertexBuffer + size, coords, size);
	
	glBufferData(GL_ARRAY_BUFFER, size * 2, VertexBuffer, GL_DYNAMIC_DRAW);
...
void SpriteBatch::CreateQuad(float *vertices, int x, int y, int width, int height, const sm::Matrix& transform)
{
	...
	{
		sm::Vec3 tl(static_cast<float>(x), static_cast<float>(y + height), 0.0f);
		sm::Vec3 tr(static_cast<float>(x + width), static_cast<float>(y + height), 0.0f);
